...
static const uint64_t k_sigma_283754_100 = 334095;

static const uint64_t k_sigma_bits_283754_100 = 19;

static const double f_283754_100 = 161032854429.0;

static const uint8_t c_ringct_283754_42_100[] = {
        255, 255, 255, 255, 249,  44,  16, 158,   4, 215, 109, 210,
        255, 255, 255, 255, 242,  88,  33,  60,   9, 221, 122,  82,
        255, 255, 255, 255, 228, 176,  66, 120,  20, 117, 111,  90,
        255, 255, 255, 255, 201,  96, 132, 240,  43, 212, 201, 133,
        255, 255, 255, 255, 146, 193,   9, 224,  99,  81,  62,  79,
        255, 255, 255, 255,  37, 130,  19, 192, 245,  65,  41, 178,
        255, 255, 255, 254,  75,   4,  39, 130, 164, 253,   7, 176,
        255, 255, 255, 252, 150,   8,  79,   8,  51, 228, 224, 142,
        255, 255, 255, 249,  44,  16, 158,  28,  15, 117,   5, 189,
        255, 255, 255, 242,  88,  33,  60, 102, 189, 151,  29,  95,
        255, 255, 255, 228, 176,  66, 121, 135, 245, 226, 125,  89,
        255, 255, 255, 201,  96, 132, 245, 249, 214, 149, 221,  82,
        255, 255, 255, 146, 193,   9, 247, 155,  88, 110,   6, 212,
        255, 255, 255,  37, 130,  20,  29, 213,  93, 219,  75, 228,
        255, 255, 254,  75,   4,  40, 246,  37, 111,  99, 252, 155,
        255, 255, 252, 150,   8,  84, 214,  53, 171,   0, 235, 167,
        255, 255, 249,  44,  16, 181,  84,  22, 115,   0, 154,  92,
        255, 255, 242,  88,  33, 153,  70, 216, 186, 212,  13,  25,
        255, 255, 228, 176,  67, 237,   8,  91, 207, 177, 250, 206,
        255, 255, 201,  96, 138, 195, 251,  57,  61, 129,  76,  68,
        255, 255, 146, 193,  33,  47, 159,  58, 163,  68,  75, 236,
        255, 255,  37, 130, 112, 253, 215, 163, 103, 205, 153, 221,
        255, 254,  75,   5, 156, 117, 196, 107, 116,  87, 186, 121,
        255, 252, 150,  14,  34, 209,  96, 204,  84,  82,   3, 129,
        255, 249,  44,  39, 237,  38,  60, 162, 211,  83, 182, 145,
        255, 242,  88, 126, 119, 187,  64, 237, 151, 238,  34, 204,
        255, 228, 177, 183,  96,  56, 154,  42, 230, 214, 137,  54,
        255, 201, 102,  88,  91, 179,  62,  93,  14, 144, 208,  45,
        255, 146, 216,  85, 230,  89, 157,  71,  82, 235,  81, 142,
        255,  37, 223,  54, 153, 178,  87,  21, 124, 187,  84,  39,
        254,  76, 120,  73,  14, 153, 226, 115, 225, 238,  20, 129,
        252, 155, 213, 136, 156, 222, 155,  70,   2,   7, 145,  55,
        249,  67,  43,  65,  58,  96, 213,  51,  85, 204, 227, 160,
        242, 179, 187, 196, 143,   3, 231, 168, 175, 227,   4,  46,
        230,  24,  77,  47, 188, 252,  33,  75, 228, 191, 179, 128,
        206, 207, 173,   0,  87, 206,  59, 177,  57,  98, 133,  46,
        167,  18, 226, 229, 200, 199,  94,  66, 155, 192, 181, 239,
        109,   9, 165, 108, 125,  94, 221, 196, 253,  39, 177,  46,
        46, 113,  55,  59, 111,  72, 161,  27, 166, 190, 109, 240,
        8, 108, 225, 235,  42,  96, 239, 161, 219, 105, 216,  85,
        0,  70, 252, 110,  24, 113,   9,  73, 136, 126,  41,   2,
        0,   0,  19, 175,   5,  30,  77,  72, 161, 248, 200, 143,
};
...
const uint8_t* get_table(uint32_t sigma, uint32_t ell, uint32_t precision){
  ...
  if (precision != 64 && precision != 128 && precision != 100) {
    return NULL; 
  }
  switch(sigma) {
  case 100:
...
uint64_t get_k_sigma(uint32_t sigma, uint32_t precision){
  if (precision != 64 && precision != 128 && precision != 100) {
    return 0;
  }
  switch(sigma){
  ...
  case 283754:
    return k_sigma_283754_100;
  default:
    return 0; 
  }
}

uint64_t get_k_sigma_bits(uint32_t sigma, uint32_t precision){
  if (precision != 64 && precision != 128 && precision != 100) {
    return 0;
  }
  switch(sigma){
  ...
  case 283754:
    return k_sigma_bits_283754_100;
  default:
    return 0;
  }
}
