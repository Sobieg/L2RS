...
typedef struct sampler_s {
  ...
  uint64_t sigma;        /* the standard deviation of the distribution */
  uint64_t ell;          /* rows in the table     */
  uint64_t precision;    /* precision used in computing the tables */
  uint64_t columns;      /* columns = precision/8 */
  uint64_t k_sigma;      /* k_sigma = ceiling[ sqrt(2*ln 2) * sigma ]  */
  uint64_t k_sigma_bits; /* number of significant bits in k_sigma */
} sampler_t;
